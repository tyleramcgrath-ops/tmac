...
export function GlassWall() {
  ...
  const verticals = useMemo(() => {
    ...
    for (let i = 0; i <= n; i++) {
      const a = NORTH_START + (i / n) * ARC
      arr.push({ x: Math.cos(a) * ROOM_R, z: Math.sin(a) * ROOM_R, ry: -a, key: i })
    }
    return arr
  }, [])

  // Horizontal transom rings at two heights.
  const transoms = [GLASS_H * 0.42, GLASS_H * 0.8]
  ...
  return (
    <group>
      ...
      {/* Vertical mullions */}
      {verticals.map((v) => (
        <mesh key={v.key} position={[v.x, GLASS_H / 2, v.z]} rotation-y={v.ry} material={brass} castShadow>
          <boxGeometry args={[0.09, GLASS_H, 0.16]} />
        </mesh>
      ))}

      {/* Horizontal transoms — full structural rings, laid flat */}
      {transoms.map((h) => (
      ...
      {[0.06, GLASS_H].map((h) => (
        <mesh key={h} position-y={h} rotation-x={Math.PI / 2} material={brass}>
          <torusGeometry args={[ROOM_R, 0.085, 12, 120]} />
        </mesh>
      ))}
    </group>
  )
