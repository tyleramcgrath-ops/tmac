...
import { CheckCircle2, Sparkles, ClipboardCheck, Check, HelpCircle, AlertTriangle, ChevronDown } from 'lucide-react'
...
export function BriefingHistory({ items }: { items: HistoryItem[] }) {
  ...
  return (
    <section aria-labelledby="history-heading" className="ns-fade-in">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="history-list"
        className="ns-touch flex w-full items-center justify-between gap-3 text-left"
      >
        <div>
          <h2 id="history-heading" className="text-sm font-medium text-white">Past checks</h2>
          <p className="mt-0.5 text-xs text-[var(--rf-muted)]">{items.length} earlier check{items.length === 1 ? '' : 's'} — what North Star found each time</p>
        </div>
        <ChevronDown className={`h-4 w-4 shrink-0 text-[var(--rf-faint)] transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>

      <div className={`ns-expand ${open ? 'ns-expand-open' : ''}`} id="history-list">
        <div>
