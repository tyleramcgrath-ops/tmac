...
/**
 * Pages sitting on an old slug, with the new name they would take.
 *
 * Renaming only ever applies to a page the theme itself owns — identified by
 * its page template, never by its slug — so a page somebody else made at
 * /about/ is never touched. WordPress records the old slug as it saves, and
 * redirects it to the new URL afterwards, so existing links keep working.
 *
 * @return array key => array( page, slug, title )
 */
function mcg_pages_to_rename() {
	$slugs = mcg_slugs();
	$map   = mcg_pages_map();
	$out   = array();

	foreach ( array_keys( mcg_templates() ) as $key ) {
		$id = mcg_page_id( $key );
		if ( ! $id ) {
			continue;
		}
		...
		// Somebody else is already on the slug we would move to.
		$taken = get_page_by_path( $slug );
		if ( $taken && (int) $taken->ID !== $id ) {
			continue;
		}
		...
		$out[ $key ] = array(
			'page'  => $page,
			'slug'  => $slug,
			'title' => isset( $map[ $slug ]['title'] ) ? $map[ $slug ]['title'] : $page->post_title,
		);
	}
...
/** Apply those renames. */
function mcg_rename_pages() {
	foreach ( mcg_pages_to_rename() as $item ) {
		wp_update_post( array(
			'ID'         => $item['page']->ID,
			'post_name'  => $item['slug'],
 ...
 * Offer the rename.
 *
 * Activation names the pages it creates; it has never renamed pages that were
 * already there, which is why changing the slugs in the theme appeared to do
 * nothing on a site that had already been set up.
 */
function mcg_rename_notice() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	$pending = mcg_pages_to_rename();
	if ( ! $pending ) {
		return;
	}

	$url = wp_nonce_url( admin_url( 'themes.php?mcg_rename=1' ), 'mcg_rename' );
	?>
	<div class="notice notice-info">
		<p><strong><?php esc_html_e( 'McGrath Chrome:', 'mcgrath-chrome' ); ?></strong>
			<?php esc_html_e( 'these pages are still on their old address. WordPress will redirect the old one to the new one, so nothing that links to them breaks.', 'mcgrath-chrome' ); ?>
		</p>
		<ul style="list-style:disc;margin-left:20px;">
			<?php foreach ( $pending as $item ) : ?>
				<li>
					<code>/<?php echo esc_html( $item['page']->post_name ); ?>/</code>
					&rarr; <code>/<?php echo esc_html( $item['slug'] ); ?>/</code>
					<?php if ( $item['title'] !== $item['page']->post_title ) : ?>
						<?php
						printf(
							/* translators: 1: current page title, 2: new page title */
							esc_html__( ' (and the title from "%1$s" to "%2$s")', 'mcgrath-chrome' ),
		...
		<p>
			<a class="button button-primary" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Rename them', 'mcgrath-chrome' ); ?></a>
			<?php esc_html_e( 'Or leave them exactly as they are — every link in the theme finds its page by template, not by address.', 'mcgrath-chrome' ); ?>
		</p>
	</div>
	<?php
}
add_action( 'admin_notices', 'mcg_rename_notice' );

/** Handle the rename link. */
function mcg_maybe_rename() {
	if ( ! isset( $_GET['mcg_rename'] ) || ! current_user_can( 'manage_options' ) ) {
		return;
	}
	check_admin_referer( 'mcg_rename' );
	mcg_rename_pages();
	wp_safe_redirect( admin_url( 'themes.php' ) );
	exit;
}
add_action( 'admin_init', 'mcg_maybe_rename' );
