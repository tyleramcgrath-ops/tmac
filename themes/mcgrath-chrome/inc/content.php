 ...
 *
 * Two modes, and the theme refuses to blur them. With "verified" off, or with
 * any of the three figures missing, the section labels itself an illustrative
 * project and lists deliverables — statements of what the work involves, which
 * are true regardless of client. Only with the switch on and all three figures
 * and a timeframe filled in does it present numbers as results.
 */
function mcg_case() {
	$metrics = array();
	for ( $i = 1; $i <= 3; $i++ ) {
		$value = trim( (string) mcg_opt( 'mcg_case_m' . $i . 'v', '' ) );
		$label = trim( (string) mcg_opt( 'mcg_case_m' . $i . 'l', '' ) );
		if ( '' !== $value && '' !== $label ) {
			$metrics[] = array( 'value' => $value, 'label' => $label );
		}
	}

	$timeframe = trim( (string) mcg_opt( 'mcg_case_timeframe', '' ) );
	$client    = trim( (string) mcg_opt( 'mcg_case_client', '' ) );

	// Every condition has to hold. A half-filled form falls back to illustrative
	// rather than showing two real numbers and one invented one.
	$verified = mcg_opt( 'mcg_case_verified', false )
		&& 3 === count( $metrics )
		&& '' !== $timeframe
		&& '' !== $client;

	// These carry their defaults here, not only in the Customizer: get_theme_mod
	// returns the fallback it is handed when no value has been saved, so an
	// empty fallback would leave a brand new site with no strip at all.
	$defaults = array(
		'd1' => __( 'Crawl, speed and index health repaired first', 'mcgrath-chrome' ),
		'd2' => __( 'Service and location pages written, not spun', 'mcgrath-chrome' ),
		'd3' => __( 'Profile, citations and schema brought into line', 'mcgrath-chrome' ),
	);

	$deliverables = array();
	foreach ( $defaults as $key => $default ) {
		$text = trim( (string) mcg_opt( 'mcg_case_' . $key, $default ) );
		if ( '' !== $text ) {
			$deliverables[] = $text;
		}
	}
	...
	return array(
		...
		'label'        => $verified ? $client : __( 'Illustrative Project', 'mcgrath-chrome' ),
		'head'         => mcg_opt( 'mcg_case_head', 'The shape a local search engagement takes' ),
		'meta'         => mcg_opt( 'mcg_case_meta', 'Jupiter, Florida · SEO, web design and AI visibility' ),
		'summary'      => mcg_opt( 'mcg_case_summary', 'A local business usually arrives with a site that is slow on a phone, service pages too thin to rank and a Google Business Profile nobody has touched since it was claimed. The work runs in that order: repair what blocks crawling and speed, write the pages that answer what buyers actually search for, then align the profile, citations and schema that decide the map.' ),
		'timeframe'    => $timeframe,
		'shot_url'     => trim( (string) mcg_opt( 'mcg_case_shot_url', '' ) ),
		'metrics'      => $verified ? $metrics : array(),
		'deliverables' => $deliverables,
	);
}

/**
 * The project screenshot, if one has been supplied.
 *
 * Nothing stands in for it. A drawn mock-up of a website that does not exist
 * is the thing that costs a case study its credibility, so when the file is
 * absent the section simply runs one column wide.
 *
 * @return string|false URI, or false when no screenshot has been added.
 */
function mcg_case_shot() {
	$dir = get_template_directory() . '/assets/img/';
	$uri = get_template_directory_uri() . '/assets/img/';

	foreach ( array( 'webp', 'jpg', 'jpeg', 'png' ) as $ext ) {
		if ( file_exists( $dir . 'case-shot.' . $ext ) ) {
			return $uri . 'case-shot.' . $ext;
		}
	}
