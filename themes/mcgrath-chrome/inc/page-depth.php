		...
		'seo'       => array(
			...
			array(
				...
				'subs'  => array(
					...
					array(
						'h3' => 'Why rankings move when you changed nothing',
						'p'  => array(
							'Three things move a position without you touching the site. Google changes how it weighs something, which happens constantly and is mostly invisible. A competitor publishes or earns links. Or the search itself changes, because what people mean by a phrase drifts with the season and the news.',
							'This is why a single week is noise and a quarter is a signal, and why a report that treats every small movement as a result of last month\'s work is telling you a story rather than reading the data.',
						),
					),
				),
			),
		...
		'aeo'       => array(
			...
			array(
				...
				'subs'  => array(
					...
					array(
						'h3' => 'Why the same question gives different answers',
						'p'  => array(
							'Ask a model the same buying question twice and you can get two different shortlists. Answers vary between sessions, between phrasings, and sharply whenever a model is updated. None of that is a fault in the tracking; it is the nature of the channel.',
							'It is also why a single run proves nothing and why the question set stays fixed and monthly. The aim is a trend you can act on, not a screenshot of one good answer, and it is a noisier signal than rank tracking will ever be.',
						),
					),
				),
			),
		...
		'about'     => array(
			...
			array(
				...
				'subs'  => array(
					...
					array(
						'h3' => 'What happens when we disagree',
						'p'  => array(
							'You win, after you have heard the argument, and that is not a courtesy. You know your buyers, your margins and which jobs you actually want more of, and none of that is visible in a tool. If I think a direction is wrong I will say so once, clearly, with the reasoning, and then do it your way.',
							'The exception is anything that would put the site at risk, such as changing URLs without redirects or removing pages that currently earn traffic. There the answer is no, and the reason is that the damage outlasts the disagreement by a long way.',
						),
					),
				),
			),
		...
		'contact'   => array(
			...
			array(
				...
				'subs'  => array(
					...
					array(
						'h3' => 'What the audit will not tell you',
						'p'  => array(
							'It will not tell you what a competitor is spending, whether their results come from real work or from a domain that has simply been around longer, or what any of it will be worth to you in revenue next quarter. Anybody presenting those as facts is presenting estimates.',
							'It also will not promise a position or a date. What it does give you is an accurate picture of where you stand, what is broken, and which three things are worth doing first, which is the part you can actually act on.',
						),
					),
				),
			),
			...
			array(
				...
				'subs'  => array(
					...
					array(
						...
						'p'  => array(
							'Paid media across several platforms, a large monthly content volume, and anything needing a team to absorb a sudden deadline. One person has a ceiling, and the honest move is to name it here rather than let you find it in month four.',
							'If that is what the job needs, you will be told on the first call and pointed somewhere better. It has cost work and it is still the right answer, because the alternative is taking money for something that was never going to fit.',
						),
					),
				),
			),
