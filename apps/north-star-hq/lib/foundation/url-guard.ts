// SSRF guard. Single source of truth for whether an outbound fetch target is
// safe. Every server-side fetch of a tenant-supplied URL — currently just
// WordPress execution — must pass through isSafeFetchTarget.
//
// Ported verbatim from the root RankForge app's app/api/seo-scan/url-guard.ts
// (Phase D.6 P4 there), which this app has no equivalent of yet: north-star-hq
// never made an outbound request to a URL a user supplied until real WordPress
// execution needed one (see wp-execution.ts). No behavior changed in the port.
//
...
// and no way to weaken the guard at runtime. check-wordpress.mts registers a
// trusted local test-double host so it can exercise the real HTTP transport
// over loopback without changing behavior for any real target.
