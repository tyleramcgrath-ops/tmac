...
// fully unit-testable without a real clock or a live crawl. Ported from the
// root app's `lib/foundation/scheduler/engine.ts` (see that repo's
// SCHEDULER_DESIGN.md for the model) — the `rf_jobs`/`rf_schedules` tables and
// FoundationStore methods here are identical, this app just hadn't wired a
// runner to them yet. The cron-triggered HTTP route calls `tick()`.
