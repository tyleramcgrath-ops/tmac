// Cron-triggered scheduler runner (North Star HQ). One pass = reap stale locks
...
// .github/workflows/scheduler-cron-north-star.yml polls this on a schedule
// instead — the same workaround the root RankForge app uses for its own
// /api/internal/cron (see that file and SCHEDULER_DESIGN.md §7). If the
// project ever moves to Vercel Pro, a vercel.json `crons` entry hitting this
// same route is a drop-in replacement — the route itself doesn't care who
// calls it.
