...
export const POST = handled(async (request) => {
  ...
  // Seed a sample project so Headquarters has something real to show on first
  // login instead of an empty state — see lib/foundation/seed.ts.
  await seedSampleProject(store, org.id, user.id)
