// Connect-Google — step 2 (Phase H, ported from the root RankForge app). Google
// redirects the browser here with a one-time `code` and our signed `state`. We
// verify the state (unforgeable + bound to the authorizing user), exchange the
// code for tokens, and persist them ENCRYPTED per (project, kind). No token is
// ever written to a log or returned to the client — only a redirect back to the
// room with a status the wizard/settings surface can read.
//
...
// signed state, not the URL. Unlike the root app (which has a /projects/[id]
// page with tabs), North Star is a single room at "/" — the status lands as a
// query param there instead of a project sub-route.
...
function backTo(request: Request, status: string, reason?: string): Response {
  const origin = new URL(request.url).origin
  const dest = `${origin}/?google=${status}`
...
export const GET = handled(async (request) => {
  ...
  if (!state) return backTo(request, 'error', 'Invalid or expired authorization state.')
  if (oauthError) return backTo(request, 'error', oauthError)
  if (!code) return backTo(request, 'error', 'Missing authorization code.')

  // The session user must match the user who started the flow.
  const user = await requireUser(request)
  if (user.id !== state.userId) return backTo(request, 'error', 'Session does not match the authorization request.')
  const { project } = await requireProjectRole(user, state.projectId, 'admin')

  const config = googleOAuthConfig()
  if (!config) return backTo(request, 'error', 'Google OAuth is not configured.')
  ...
  try {
    ...
    return backTo(request, 'error', err instanceof Error ? err.message : 'Token exchange failed.')
  }
