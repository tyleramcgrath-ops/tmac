...
const { enterApp, serveStatic, serveAccount } = require('./enter-app.js');
...
(async () => {
  ...
  const srv = http.createServer((q, r) => { const pn = require('url').parse(q.url).pathname;
    if (serveAccount(pn, r)) return;
    if (serveStatic(pn, r, root)) return; r.setHeader('content-type', 'text/html'); r.end(fs.readFileSync(path.join(root, 'index.html'))); });
