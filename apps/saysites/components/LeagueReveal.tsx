...
// Last week's result arrives sealed. The owner breaks the seal to see where
// they finished; after that it stays open in this browser.
export function LeagueReveal({ siteId, week, place, of, league, titles }: { siteId: string; week: string; place: string; of: number; league: string; titles: string[] }) {
  ...
  return (
    <div className={`reveal is-${state}`}>
      <div className="reveal-card" aria-live="polite">
        <span className="stat-label">Last week · {league}</span>
        <strong className="reveal-place">{place}</strong>
        <span className="muted small">of {of}</span>
        {titles.length > 0 && (
      ...
      {state !== 'open' && (
        <button type="button" className="reveal-envelope" onClick={open} disabled={state === 'opening'} aria-label="Open last week's result">
          ...
          <span className="reveal-cta">Last week’s result is in · Break the seal</span>
        </button>
      )}
    </div>
  )
